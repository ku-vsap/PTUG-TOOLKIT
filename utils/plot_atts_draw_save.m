function plot_atts_draw_save(num_tracker, plot_style, ave_success_rate_plot, idx_seq_set, rank_num, ...
                        ranking_type, rank_idx, name_tracker_all, threshold_set, title_name, ...
                        x_label_name, y_label_name, fig_name, save_fig_path, save_fig_suf, att_idx)
% plot and save curves
perf = zeros(1, num_tracker);
for i=1:num_tracker
    %each row is the sr plot of one sequence
    tmp = ave_success_rate_plot(i, idx_seq_set, att_idx, :);
    aa  = reshape(tmp, [numel(idx_seq_set), size(ave_success_rate_plot, 4)]);
    aa  = aa(sum(aa,2)>eps, :);
    bb  = mean(aa);
    switch ranking_type
        case 'AUC'
...
for k = index_sort(1:rank_num)
    ...
    tmp = ave_success_rate_plot(k, idx_seq_set,att_idx, :);
    aa  = reshape(tmp, [numel(idx_seq_set), size(ave_success_rate_plot, 4)]);
    aa  = aa(sum(aa,2)>eps, :);
    bb  = mean(aa);
    ...
    %grid minor
    if k == index_sort(1)
        set(gca,'GridLineStyle', ':', 'GridColor', 'k', 'GridAlpha', 1, 'LineWidth', 1.2,'xtick',xp, 'FontSize',20);
    end
    i = i + 1;
end
...
legend(tmpName, 'Interpreter', 'none', 'fontsize', font_size, 'Location', legend_position);
