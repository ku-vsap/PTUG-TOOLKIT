function eval_atts(seqs, trackers, eval_type, name_tracker_all, tmp_mat_path, path_anno, rp_all, att_idx, norm_dst)
% evaluate each tracker
num_tracker = numel(trackers);

threshold_set_overlap = 0:0.05:1;
threshold_set_error   = 0:50;
% threshold_set_overlap = 0:0.2:1;
threshold_set_error   = 0:50;
if norm_dst
...
for i = 1:numel(seqs) % for each sequence
    ...
    att_annos    = dlmread([path_anno 'att/' s '.txt']);
...
%         res = dlmread([rp_all t.name '/' s]); % Need to uncomment the above line of 
...
%         res = results{1};
        ...
        att_anno = att_annos(:, att_idx);
        [err_coverage, err_center] = calc_seq_err_robust(res, anno, absent_anno, att_anno, norm_dst);
        ...
        ave_success_rate_plot(k, i, att_idx, :)     = success_num_overlap/(len_all + eps);
        ave_success_rate_plot_err(k, i, att_idx, :) = success_num_err/(len_all + eps);
...
dataName1 = [tmp_mat_path 'aveSuccessRatePlot_' num2str(num_tracker) ...
    'att_' num2str(att_idx) '_alg_overlap_' eval_type '.mat'];
save(dataName1, 'ave_success_rate_plot', 'name_tracker_all');

dataName2 = [tmp_mat_path 'aveSuccessRatePlot_' num2str(num_tracker) ...
    'att_' num2str(att_idx) '_alg_error_' eval_type '.mat'];
